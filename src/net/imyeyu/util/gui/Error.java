...
public class Error extends JDialog implements ActionListener {
	...
	public Error() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(Error.class.getResource(YeyuUtils.RES_PATH + "error.png")));
		setSize(700, 400);
		setUndecorated(true);
		...
		label.setIcon(new ImageIcon(Error.class.getResource(YeyuUtils.RES_PATH + "error.png")));
		...
		if (object == report) {
			try {
				...
				if (i == JOptionPane.YES_OPTION) {
					YeyuUtils.network().doPost("https://www.imyeyu.net/java/report", param);
